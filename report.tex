...
% %  With RNNss, each conditional probability is modeled as,
...
% 
% \begin{figure}
%     \includegraphics[width=.99\linewidth]{img/contextres.png}  
%     \caption{visualization of context} 
%     \label{fig:contextres}
% \end{figure}
% 
% A visualization of the context vector is shown in the Fig. \ref{fig:contextres}.
% 
% The \cite{bahdanau2014neural} paper also made use of Bi-directional RNNs. Like RNNs the bi-directional RNNs generate $h_t$ based on the future states. This is particularly useful in machine translation where the current word may also be influenced by next words. Bi-directional LSTM has two cell memory states instead on one on the vanilla LSTMs as shown in \ref{fig:birnn}.
%  
% \begin{figure}
%     \includegraphics[width=.99\linewidth]{img/birnn.png}  
%     \caption{Bi-directional LSTM} 
%     \label{fig:birnn}
% \end{figure}
% 
% 
% \subsubsection{Seq2Seq Learning}

% While it seems machine translation is unique problem involving to very different problem of understanding a source language sentence and then generating a target language, the paper \cite{sutskever2014sequence} treated the problem of machine translation just a special case of more generic sequence to sequence model where a multi layered LSTM network is trained to map an input sequence $X$ to an output sequence $Y$. This is applicable to many problems like voice recognition where sound signals are mapped to language works, hand gestures are mapped to written digits, etc. Machine translation is a special case as follows,
% 
% $$ p(y_1, \cdots, y_T| x_1, \cdots, x_T) = \prod_{t=1}^{T} p(y_t| v, y_1, \cdots, y_{t-1})$$
% where $y$ is the target sentence, $x$ is the source sentence, and $v$ is the information contained in the LSTM cell memory.
% This paper achieved the state-of-the-art without making any additional handling for machine translation. 
% The core idea is use to a simple LSTM encoder-decoder architecture as shown in  \ref{fig:s2s} but a more powerful model as shown in  \ref{fig:s2sbig}
...
% % 	\caption{Seq2Seq more powerful model} 
% 	\label{fig:s2sbig}
% %     \end{figure}
